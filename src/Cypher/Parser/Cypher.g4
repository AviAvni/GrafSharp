...
match : ( OPTIONAL SP )? MATCH SP? pattern ( hint )* ( SP? where )? ;
...
createUnique : CREATE SP UNIQUE SP? pattern ;
...
foreach : FOREACH SP? '(' SP? variable SP IN SP expression SP? '|' ( SP updatingClause )+ SP? ')' ;
...
start : START SP startPoint ( SP? ',' SP? startPoint )* where? ;
...
stringListNullOperatorExpression : propertyOrLabelsExpression ( ( SP? '[' expression ']' ) | ( SP? '[' expression? '..' expression? ']' ) | ( ( regularExpression | ( SP IN ) | ( SP STARTS SP WITH ) | ( SP ENDS SP WITH ) | ( SP CONTAINS ) ) SP? propertyOrLabelsExpression ) | ( SP IS SP NULL ) | ( SP IS SP NOT SP NULL ) )* ;
...
reduce : REDUCE SP? '(' variable '=' expression ',' idInColl '|' expression ')' ;
